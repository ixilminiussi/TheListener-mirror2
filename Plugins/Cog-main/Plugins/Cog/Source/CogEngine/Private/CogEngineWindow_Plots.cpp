...
void FCogEngineWindow_Plots::RenderMenu(FCogDebugTracker& InTracker)
{
	if (ImGui::BeginMenuBar())
	{
		...
		if (ImGui::BeginMenu("Options"))
		{
			...
			constexpr ImGuiColorEditFlags ColorEditFlags = ImGuiColorEditFlags_NoInputs |
...
void FCogEngineWindow_Plots::RenderEntryName(FCogDebugTracker& InTracker, const int Index, FCogDebugTrack& Entry)
{
	...
	if (ImGui::Selectable(COG_TCHAR_TO_CHAR(*Entry.Id.ToString()), IsAssignedToGraph,
...
void FCogEngineWindow_Plots::RenderPlots(FCogDebugTracker& InTracker)
{
	if (ImGui::BeginChild("Graph", ImVec2(0, -1)))
	{
		...
		{
			for (int32 GraphIndex = 0; GraphIndex < Config->NumGraphs && GraphIndex <
			     UCogEngineConfig_Plots::MaxNumGraphs; ++GraphIndex)
			{
				...
				if (ImPlot::BeginPlot("##Plot", ImVec2(-1, 250)))
				{
					...
					for (int32 YAxisIndex = 0; YAxisIndex <= (ImAxis_Y3 - ImAxis_Y1); ++YAxisIndex)
					{
						...
						if (IsAxisVisible)
						{
							ImPlotAxisFlags Flags = IsAssigned
					...
					if (Config->ShowTimeBarAtGameTime || Config->ShowTimeBarAtCursor)
					{
						...
						{
							PlotDrawList->AddLine(ImVec2(ImGui::GetMousePos().x, PlotTop),
							                      ImVec2(ImGui::GetMousePos().x, TimeBarBottom),
						...
						if (Config->ShowTimeBarAtGameTime && InTracker.Pause)
						{
							...
							PlotDrawList->AddLine(ImVec2(TimeBarX, PlotTop), ImVec2(TimeBarX, TimeBarBottom),
...
void FCogEngineWindow_Plots::RenderValues(FCogDebugPlotTrack& Timeline, const char* Label) const
{
	...
	ImPlot::PlotLine(Label, &Timeline.Values[0].x, &Timeline.Values[0].y, Timeline.Values.size(), ImPlotLineFlags_None,
	                 Timeline.ValueOffset, 2 * sizeof(float));
...
//--------------------------------------------------------------------------------------------------------------------------
void FCogEngineWindow_Plots::RenderEvents(FCogDebugEventTrack& InTrack, const char* InLabel, const ImVec2& InPlotMin,
                                          const ImVec2& InPlotMax) const
{
	const ImVec2 Mouse = ImGui::GetMousePos();
	ImDrawList* PlotDrawList = ImPlot::GetPlotDrawList();
	...
	ImPlot::PlotLine(InLabel, &DummyData[0].x, &DummyData[0].y, DummyData.size(), InTrack.EventOffset,
	                 2 * sizeof(float));
	...
	for (const FCogDebugEvent& Event : InTrack.Events)
	{
		...
		if (IsInstant)
		{
			...
			PlotDrawList->AddText(ImVec2(PosMid.x + 15, PosMid.y - 6), IM_COL32(255, 255, 255, 255),
			                      COG_TCHAR_TO_CHAR(*Event.DisplayName));

			if ((Mouse.x > PosMid.x - Radius) && (Mouse.x < PosMid.x + Radius) && (Mouse.y > PosMid.y - Radius) && (
				Mouse.y < PosMid.y + Radius))
			{
				HoveredEvent = &Event;
			}
		}
		else
		{
			...
			const ImDrawFlags Flags = Event.EndTime == 0.0f
			...
			PlotDrawList->AddText(ImVec2(PosMid.x + 5, PosMid.y - 7), IM_COL32(255, 255, 255, 255),
			                      COG_TCHAR_TO_CHAR(*Event.DisplayName));
...
//--------------------------------------------------------------------------------------------------------------------------
void FCogEngineWindow_Plots::AssignToGraphAndAxis(FCogDebugTracker& InTracker, const FName InName,
                                                  const int32 InGraphIndex, const ImAxis InYAxis)
{
...
void FCogEngineWindow_Plots::UnassignToGraphAndAxis(FCogDebugTracker& InTracker, const FName InName)
{
	...
	const int32 Index = GraphInfo.Entries.IndexOfByPredicate([InName](const auto& InEntry)
	{
