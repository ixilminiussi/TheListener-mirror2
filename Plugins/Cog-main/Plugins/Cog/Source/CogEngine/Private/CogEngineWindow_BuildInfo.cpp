...
void FCogEngineWindow_BuildInfo::RenderTick(float DeltaTime)
{
	...
	DrawList->AddRectFilled(AlignedPos, AlignedPos + RectSize, FCogImguiHelper::ToImU32(Config->BackgroundColor),
	                        Config->Rounding);
	DrawList->AddRect(AlignedPos, AlignedPos + RectSize, FCogImguiHelper::ToImU32(Config->BorderColor),
	                  Config->Rounding);
...
void FCogEngineWindow_BuildInfo::RenderContent()
{
	...
	if (ImGui::BeginChild("Settings", ImVec2(-1, 100 * GetDpiScale()),
	                      ImGuiChildFlags_FrameStyle | ImGuiChildFlags_Borders | ImGuiChildFlags_ResizeY))
	{
		if (ImGui::Checkbox("Branch Name", &Config->ShowBranchName)) { BuildText(); }
		if (ImGui::Checkbox("Build Date", &Config->ShowBuildDate)) { BuildText(); }
		if (ImGui::Checkbox("Build Version", &Config->ShowBuildVersion)) { BuildText(); }
...
void FCogEngineWindow_BuildInfo::BuildText()
{
	...
	{
		S.Append(BuildSettings::GetBranchName());
		S.Append(Config->Separator);
	}
	if (Config->ShowBuildDate)
	{
		S.Append(TEXT(BUILD_TIMESTAMP));
		S.Append(Config->Separator);
	}
	if (Config->ShowBuildVersion)
	{
		S.Append(GetProjectVersion());
		S.Append(Config->Separator);
	}
	if (Config->ShowBuildConfiguration)
	{
		S.Append(LexToString(FApp::GetBuildConfiguration()));
		S.Append(Config->Separator);
	}
	if (Config->ShowBuildTargetType)
	{
		S.Append(LexToString(FApp::GetBuildTargetType()));
		S.Append(Config->Separator);
	}
	if (Config->ShowBuildUser)
	{
		S.Append(BuildSettings::GetBuildUser());
		S.Append(Config->Separator);
	}
	if (Config->ShowBuildMachine)
	{
		S.Append(BuildSettings::GetBuildMachine());
		S.Append(Config->Separator);
	}
	if (Config->ShowCurrentChangelist)
	{
		S.Appendf(TEXT("%d"), BuildSettings::GetCurrentChangelist());
		S.Append(Config->Separator);
	}
	if (Config->ShowCompatibleChangelist)
	{
		S.Appendf(TEXT("%d"), BuildSettings::GetCompatibleChangelist());
		S.Append(Config->Separator);
	}
...
FString FCogEngineWindow_BuildInfo::GetProjectVersion()
{
