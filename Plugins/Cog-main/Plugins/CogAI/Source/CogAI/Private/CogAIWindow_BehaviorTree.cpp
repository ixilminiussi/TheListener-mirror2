...
void FCogAIWindow_BehaviorTree::RenderContent()
{
	...
	if (ImGui::BeginMenuBar())
	{
		...
		{
			ImGui::ColorEdit4("Active Color", &Config->ActiveColor.X,
			                  ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_AlphaPreviewHalf);
			ImGui::ColorEdit4("Inactive Color", &Config->InactiveColor.X,
			                  ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_AlphaPreviewHalf);
			ImGui::EndMenu();
		}
	...
	for (const UBTNode* ParentNode = BehaviorTreeComponent->GetActiveNode(); ParentNode != nullptr; ParentNode =
	...
	{
		if (ImGui::CollapsingHeader(StringCast<ANSICHAR>(*GetNameSafe(CurrentTree)).Get(), nullptr,
...
//--------------------------------------------------------------------------------------------------------------------------
void FCogAIWindow_BehaviorTree::RenderNode(UBehaviorTreeComponent& BehaviorTreeComponent, UBTNode* Node,
                                           bool OpenAllChildren)
{
	...
	if (ShowNode)
	{
		...
		{
			OpenChildren = ImGui::TreeNodeEx(
				"##Node", ImGuiSelectableFlags_AllowOverlap | ImGuiTreeNodeFlags_SpanFullWidth);
		}
		else
		{
			ImGui::TreeNodeEx(
				"##Node",
				ImGuiTreeNodeFlags_Leaf | ImGuiTreeNodeFlags_NoTreePushOnOpen | ImGuiSelectableFlags_AllowOverlap |
		...
		if (FCogWidgets::BeginItemTableTooltip())
		{
			if (ImGui::BeginTable("Effect", 2, ImGuiTableFlags_Borders))
			{
				...
				uint8* NodeMemory = BehaviorTreeComponent.GetNodeMemory(
					Node, BehaviorTreeComponent.GetActiveInstanceIdx());
				Node->DescribeRuntimeValues(BehaviorTreeComponent, NodeMemory, EBTDescriptionVerbosity::Detailed,
				                            RunTimeValues);
		...
		//------------------------
		ImGui::SameLine();
		const ImVec4 NameColor = IsActive
